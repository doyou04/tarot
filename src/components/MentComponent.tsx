...
export default function MentComponent({
  ...
  return (
    <motion.div
      ...
      <h2 className='text-2xl text-amber-100 mb-3 text-center'>
        {(selectedMenu === 'love' &&
          `누군가의 마음을 사냥하고 싶은 집사가 왔구냥?`) ||
          (selectedMenu === 'career' &&
            `거친 사냥터에서 살아남고 싶은 집사로구냥.`) ||
          (selectedMenu === 'money' &&
            `황금 츄르를 가득 모으고 싶은 집사로구냥.`) ||
          `건강이 궁금해서 나를 찾아왔구냥?`}
      </h2>
      <h2 className='text-2xl text-amber-100 text-center'>
        마음속으로 {menuName}에 대한 질문을 떨올려라냥
      </h2>
      ...
      <motion.button
        ...
        className='group relative mt-20 mx-auto px-12 py-4 bg-slate-900/80 border-2 border-amber-400/50 rounded-full flex items-center gap-3 overflow-hidden'>
        {/* 버튼 내부 반짝임 효과 */}
